<script>
  import Util from '../components/Util.svelte'
</script>

<svelte:head>
  <title>utils by Stone Sha</title>
</svelte:head>

<h1 class="mb-4 font-mono font-bold text-6xl text-center text-slate-200">utils.stonesha.com</h1>

<div class="grid grid-cols-4 gap-2">
  <Util title='JSON Validator and Formatter' slug='json-validator-and-formatter'/>
  <Util title='URL Encoder and Decoder' slug='url-encoder-and-decoder'/>
  <Util title='Video Media Extractor' slug='video-media-extractor'/>
</div>
