<script lang="ts">
		...
		classes: [
			{
				...
				sub_classes: {
					...
					'Path of the Storm Herald (XGtE)': ['Desert (XGtE)', 'Sea (XGtE)', 'Tundra (XGtE)'],
	...
	const generateRace = () => {
		dnd_race = dnd_data.races[Math.floor(Math.random() * dnd_data.races.length)]
	}
	...
	const generateClass = () => {
		...
		if (random_class.sub_classes[random_sub_class].length > 0) {
			...
			const random_sub_sub_class =
				sub_sub_classes[Math.floor(Math.random() * sub_sub_classes.length)]
	...
	const generateStats = () => {
		...
		for (let i = 0; i < 6; i++) {
			...
			const modifier =
				determineModifier(total) > 0
					? `+${determineModifier(total)}`
					: determineModifier(total).toString()
...
<div class="flex flex-col">
	...
	{#if rolled}
		<div class="flex flex-col md:flex-row justify-around">
			<p class="bg-slate-50 rounded-lg p-2 m-2 text-center">
				Strength: {dnd_stats.strength} ({dnd_stats.strength_modifier})
			</p>
			<p class="bg-slate-50 rounded-lg p-2 m-2 text-center">
				Dexterity: {dnd_stats.dexterity} ({dnd_stats.dexterity_modifier})
			</p>
			<p class="bg-slate-50 rounded-lg p-2 m-2 text-center">
				Constitution: {dnd_stats.constitution} ({dnd_stats.constitution_modifier})
			</p>
			<p class="bg-slate-50 rounded-lg p-2 m-2 text-center">
				Intelligence: {dnd_stats.intelligence} ({dnd_stats.intelligence_modifier})
			</p>
			<p class="bg-slate-50 rounded-lg p-2 m-2 text-center">
				Wisdom: {dnd_stats.wisdom} ({dnd_stats.wisdom_modifier})
			</p>
			<p class="bg-slate-50 rounded-lg p-2 m-2 text-center">
				Charisma: {dnd_stats.charisma} ({dnd_stats.charisma_modifier})
			</p>
		</div>
	{/if}
</div>
